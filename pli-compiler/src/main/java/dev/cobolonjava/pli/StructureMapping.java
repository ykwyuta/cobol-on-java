 ...
 * 保ったまま 1 つの要素として外の構造の対に加わる。{@code UNALIGNED} は境界合わせを 1 byte にし、
 * 構造に書けば要素が受け継ぐ。
 *
 * <p>以前は要素を隙間なく並べていた (暫定判断 P-184)。
 */
final class StructureMapping {

    /** 配置の結果。添字は宣言の並び (構造の頭が 0) である。 */
    record Result(int size, int[] offsets, int[] lengths) {
    }

    /** 対にまとめた単位。{@code start} は倍語の境界からのずれ (0〜7)。 */
    ...
    static Result map(List<PliSyntax.Decl> tree) {
        ...
        for (int i = 0; i < tree.size(); i++) {
            offsets[i] = unit.offsets().getOrDefault(i, 0);
        }
        lengths(tree, 0, tree.size(), unit, lengths);
        return new Result(unit.length(), offsets, lengths);
    }
    ...
    private static Unit element(PliSyntax.Decl decl, boolean unaligned, int index) {
        if (decl.type() == PliSyntax.Type.POINTER) {
            // POINTER の変数は指す先の記憶域そのものとして持っており、4 byte の値を置く場所がない
            throw new IllegalArgumentException("POINTER member " + decl.name()
                    + " in a structure is not supported yet (P-184)");
        }
        if (decl.type() == PliSyntax.Type.BIT && unaligned && decl.precision() % 8 != 0) {
            // UNALIGNED のビット列は前後とビット単位で詰まる。この処理系は byte 単位でしか置けない
            throw new IllegalArgumentException("unaligned BIT(" + decl.precision() + ") member "
                    + decl.name() + " is not supported yet; only multiples of 8 bits (P-184)");
        }
        return new Unit(0, length(decl), unaligned ? 1 : alignment(decl),
    ...
    private static Unit pair(Unit first, Unit second) {
        ...
        return new Unit(Math.floorMod(shifted, 8), q + second.length() - shifted,
                Math.max(first.alignment(), second.alignment()), offsets);
    }

    /** 小構造の長さは、配置で決まった最後の要素の終わりまで。 */
    private static void lengths(List<PliSyntax.Decl> tree, int index, int end, Unit root,
                                int[] lengths) {
        for (int i = index; i < end; i++) {
            PliSyntax.Decl decl = tree.get(i);
            if (decl.type() != PliSyntax.Type.GROUP) {
                lengths[i] = length(decl);
                continue;
            }
            int groupEnd = i + 1;
            while (groupEnd < end && tree.get(groupEnd).level() > decl.level()) groupEnd++;
            int last = root.offsets().get(i);
            for (int j = i + 1; j < groupEnd; j++) {
                if (tree.get(j).type() != PliSyntax.Type.GROUP) {
                    last = Math.max(last, root.offsets().get(j) + length(tree.get(j)));
                }
            }
            lengths[i] = last - root.offsets().get(i);
        }
    }
    ...
    /** ALIGNED のときの境界合わせ (LRM Table 39)。文字・PICTURE・10 進・ビット列は byte。 */
