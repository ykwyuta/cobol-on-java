...
public record CicsEnvironment(
     ...
     * 解釈はtaskのhostZoneが決めるので、ここで推測は起きない。資源の排他は1つのJVMの中で効く。
     */
    ...
    private CicsEnvironment withApplidValue(String value) {
        return new CicsEnvironment(Optional.of(value), clock, interval, mapsets, enqueues);
    }

    /** 時計だけを替えた構成。試験で時刻を固定するときに使う。 */
    public CicsEnvironment withClock(Clock value) {
        return new CicsEnvironment(applid, value, interval, mapsets, enqueues);
    }

    /** 待ちだけを替えた構成。 */
    public CicsEnvironment withInterval(CicsIntervalPort value) {
        return new CicsEnvironment(applid, clock, value, mapsets, enqueues);
    }

    /** mapsetの定義を持たせた構成。 */
    public CicsEnvironment withMapsets(BmsMapsetCatalog value) {
        return new CicsEnvironment(applid, clock, interval, Optional.of(value), enqueues);
    }

    /** 資源の排他を替えた構成。複数のJVMで分け合うときに使う。 */
    public CicsEnvironment withEnqueues(CicsEnqueuePort value) {
        return new CicsEnvironment(applid, clock, interval, mapsets, value);
    }
}
