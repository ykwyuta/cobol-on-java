...
| `cobol-compiler` | プリプロセッサ・構文解析・ASM によるコード生成 | P0-b 着手。プリプロセッサから ASM によるコード生成まで一通り実装済。`MOVE` だけの小さなプログラムが翻訳して実行できる |
...
P0-a (ランタイム先行) と V2 期待値の採取基盤を実装済み。P0-b (コンパイラ) に着手。テスト 514 件。
