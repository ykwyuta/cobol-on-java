...
| `hlasm-assembler` | HLASM の原文を読み、機械語へ組み立てて実行する (設計 27) | 増分 1 実装済。**組み立て**: 固定形式の読み取り (継続・属性参照 `L'` の読み分け・`ICTL` は断る)、`CSECT` / `DSECT` / `USING` / `DROP` / `DC` / `DS` / `EQU` / `ORG` / `LTORG` / `END`、式 (自己定義項・所在カウンタ・長さ属性・再配置属性の検査)、リテラルプール、RR / RX / RS / SI / SS 形式の機械命令 96 個と拡張ニーモニック。**実行**: 線形の番地空間 (呼ぶ側の記憶域をそのまま指し、エイリアシングを保つ)、16 本の汎用レジスタ、`EX` による自己書き換えと計算分岐、10 進命令は `cobol-runtime` の Hercules で裏づけ済みの層へ委ねる。標準リンケージ (R1 の引数表 ↔ `DataView[]`) で `CobolProgram` ABI に着地し、COBOL と同じ `CALL` で呼べる。ソース内マクロ、明示的な `COPY`、静的・変数生成の命令名からのライブラリマクロ探索、`ACTR`、`MNOTE`、マクロパラメタの `K'` / `N'` と副リスト参照、`&SYSLIST`、SETC / AIF の部分文字列、添字付き SET 記号、静的な前方参照の `T'` / `L'` / `S'` / `I'` と `D'` / `O'`、文字・算術関数、システム変数を条件付きアセンブリで扱う。知らない命令欄は読み飛ばさずに断る。`SVC` は組み立てられるがまだ実行できず、演算例外で断る。組み立ては `cobol-oracle` の `Insn` と突き合わせ済み。**実行は Hercules で合成 26 ケース (正常 19、割込み 7) を比較済み。残る命令と実資産コーパスは未了** (P-175) |
...
```
java -cp <classpath> dev.cobolonjava.hlasm.Main -d out -I asmlib -l BUMP.asm
java -cp out:<classpath> cobol.generated.BUMP
```

ソース内マクロ、基本的な条件付きアセンブリ、`COPY`、静的・変数生成の命令名からのライブラリマクロ探索に対応する。
CLI の `-I` または Maven の `src/main/asmlib` がメンバの置き場である。
`WTO` / `OPEN` の OS サービスはまだ無く、読み飛ばさずに診断を出して断る。
