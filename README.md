...
| `hlasm-assembler` | HLASM の原文を読み、z/Architecture の機械語へ組み立てる (設計 27) | 増分 1 の前半。固定形式の読み取り (継続・属性参照 `L'` の読み分け・`ICTL` は断る)、`CSECT` / `DSECT` / `USING` / `DROP` / `DC` / `DS` / `EQU` / `ORG` / `LTORG` / `END`、式 (自己定義項・所在カウンタ・長さ属性・再配置属性の検査)、リテラルプール、RR / RX / RS / SI / SS 形式の機械命令 96 個と拡張ニーモニックを実装。マクロと条件付きアセンブリは<b>持たない</b>。知らない命令欄は読み飛ばさずに断る。組み立てた機械語は、Hercules で裏づけ済みの `cobol-oracle` の `Insn` と突き合わせている |
