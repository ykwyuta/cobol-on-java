...
| `cobol-spring-boot-4-autoconfigure` | Spring Boot 4.x 固有機能を中立ポートへ接続 | Spring Boot 4.1.1 基準の `SPRING_MANAGED` Db2 UOW、初期SQL executor、非hold cursorを実装。CICS task の coordinator の自動構成と、JSON の入口 `POST /api/cics/{transid}` (Spring Security があるときだけ、P-135) を実装。driver管理 `WITH HOLD`、`STRICT` の会話ストアは未実装 |
...
CRECUST の非同期 API と LE の写し句である。`WRITE FILE` は固定長 KSDS へ鍵で書く形を、バッチと同じ索引編成の
データセットで扱う (P-131)。`INQUIRE ASSOCIATION` は task 自身の origin data に限る (P-132)。`RECEIVE MAP ... ASIS` は端末が UCTRAN でも大文字にしない。
