...
P0-a (ランタイム先行) の第 1 増分と、V2 期待値の採取基盤を実装済み。テスト 172 件。
うち 22 件は Hercules 上での実行と突き合わせる**検証レベル V2** であり、残りは V1。
V2 のうち 3 件は合成ジェネレータが生成した組み合わせ (加減算 121 件、乗算 121 件、除算 99 件)
をそれぞれ一度に検証する。
...
- 数値比較の条件コード (`CP`)
- 合成ジェネレータによる 341 通りの組み合わせ (バイト長 x 値の分類 x 演算の pairwise)
