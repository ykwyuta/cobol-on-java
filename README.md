...
P0-a (ランタイム先行) の第 1 増分と、V2 期待値の採取基盤を実装済み。テスト 143 件。
うち 9 件は Hercules 上での実行と突き合わせる**検証レベル V2** であり、残りは V1。

V2 で裏付けが取れているのは、10 進加減乗除とゼロ結果の符号、丸め (`SRP`)、
符号ニブルの受理規則、および数値編集のうちゼロ抑制・小切手保護・`CR`/`DB`・単純挿入である。
