...
P0-a (ランタイム先行) の第 1 増分と、V2 期待値の採取基盤を実装済み。テスト 169 件。
うち 19 件は Hercules 上での実行と突き合わせる**検証レベル V2** であり、残りは V1。
V2 のうち 1 件は合成ジェネレータが生成した 121 通りの組み合わせを一度に検証する。

V2 で裏付けが取れているのは以下である。

- 10 進加減乗除とゼロ結果の符号 (`AP` `SP` `MP` `DP` `ZAP`)
- 丸め (`SRP`) と符号ニブルの受理規則
- 数値編集のうちゼロ抑制・小切手保護・`CR`/`DB`・単純挿入 (`ED`)
- ゾーン10進との相互変換 (`PACK` `UNPK`)
- データ例外 (COBOL から見た `S0C7`) の発生条件
- 数値比較の条件コード (`CP`)
- 合成ジェネレータによる 121 通りの組み合わせ (バイト長 x 値の分類 x 演算の pairwise)
