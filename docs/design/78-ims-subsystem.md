...
IMS はトランザクションコード単位で FIFO 処理する。第 1 増分は**キューあたり単一コンシューマ・`prefetch=1`** を既定とする。実機の並列スケジューリングは再現しない ([P-105](../decisions/provisional.md))。
