...
IMS はトランザクションコード単位で FIFO 処理する。第 1 増分は**キューあたり単一コンシューマ**を前提とする (処理系は強制しない)。実機の並列スケジューリングは再現しない ([P-105](../decisions/provisional.md))。<b>当初ここに書いていた `prefetch=1` は実装されておらず、設定しても消費者どうしの配分は変わらないことを 2026-09-16 に測った</b>。順序を守る梃子は消費者の数だけである。
