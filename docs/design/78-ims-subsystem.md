...
キーは無期限には残せないので、**保持期間を過ぎたものは置き場を開くときに刈る** (既定 7 日、`cobol.ims.inbox-retention-days`、0 以下なら刈らない)。<b>刈ったあとにブローカがその電文を再配信すると、業務は 2 度動く</b>。日数はブローカが電文を持ちうる最長の時間より長くなければならないが、それは測っていない ([P-163](../decisions/provisional.md))。
...
そこで、**取引コードを置き場の借用の行で 1 つの領域に限り、2 つ目は起こさずに断る** ([P-167](../decisions/provisional.md))。心拍は同期点の確定に相乗りさせ、引き継がれた領域は次の同期点で競合として止まる。借用が働くのは RDB の置き場と JMS のキューがそろったときだけで、データセットの置き場では働かない。そこで<b>取引コードを名乗るキューなのに借用を持てない置き場なら、既定では領域を起こさずに断る</b> (`cobol.ims.queue.lease-required=false` で降りられる)。バッチと 1 つの JVM の中のキューは、プロセスをまたいで二重にならないので断らない。
