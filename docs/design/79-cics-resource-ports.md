...
| ステータス | 起草 (2026-09-14)。§10 の 1 (動的 PROGRAM) 、2 (ASSIGN APPLID / PROGRAM、`CicsEnvironment` の APPLID)、3 (ASKTIME / FORMATTIME、`CicsEnvironment` の時計)、4 (DELAY、`CicsIntervalPort`) を実装済み、5 (中立な合成部 `BmsSymbolicLayout` / `BmsScreenComposer` と、`SEND MAP` / `SEND TEXT` / `SEND CONTROL`、task 結果の画面)、6 (`RECEIVE MAP`、会話への画面保存、MAPFAIL) を実装済み |
