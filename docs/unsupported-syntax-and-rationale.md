...
| **CICS キューの一部 (遠隔・共有・区画外・ATI)** | `EXEC CICS WRITEQ TS QUEUE('Q') FROM(A) SYSID('S1') END-EXEC` | `WRITEQ` / `READQ` / `DELETEQ` の `TS` / `TD` は 1 つの JVM の中のキューとして実装済み（設計 82 §4・§5、P-137）。trigger level による task の開始は設計 83 (P-144)、区画外の TD (順編成のデータセット) と回復可能なキュー (STRICT の UOW に入る) は設計 85 §7 (P-148) で実装済み。`SYSID`、`NOSUSPEND`、`SET`、`ITEM` も `NEXT` も無い `READQ TS` は断る。 | 設計 85 §9 の順に SYSID / NOSUSPEND と SET を足す。 |
| **CICS START の一部 (`CHANNEL` / `USERID` と `TERMID` の併記)** | `EXEC CICS START TRANSID('TX02') CHANNEL('CH1') END-EXEC` | 端末を持たない task を起こす `START` と、`RETRIEVE`、`CANCEL REQID` は実装済み（設計 82 §6、P-138）。`PROTECT` も実装済み（P-141）。端末へ出す `TERMID` は `JdbcCicsStarts` の構成で実装済み（設計 83 §5、P-144）。`RETRIEVE WAIT` も端末へ出す task で実装済み。別の利用者で動かす `USERID` は代理の権限を確かめて実装済み（設計 84、P-145）。`USERID` と `TERMID` の併記、`CHANNEL` / `ATTACH`、POST の `CANCEL` は、振る舞いを確かめていないか task の境界と合わせた設計を持たないので断る。 | 必要な資産が見つかれば、実機の振る舞いを確かめて設計する。 |
