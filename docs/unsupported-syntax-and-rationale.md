...
| **CICS START の一部 (`USERID` / `CHANNEL`)** | `EXEC CICS START TRANSID('TX02') USERID('U1') END-EXEC` | 端末を持たない task を起こす `START` と、`RETRIEVE`、`CANCEL REQID` は実装済み（設計 82 §6、P-138）。`PROTECT` も実装済み（P-141）。端末へ出す `TERMID` は `JdbcCicsStarts` の構成で実装済み（設計 83 §5、P-144）。別の利用者で動かす `USERID`、`CHANNEL` / `ATTACH`、`RETRIEVE WAIT`、POST の `CANCEL` は、利用者の認可と task の境界と合わせた設計を持たないので断る。 | `RETRIEVE WAIT` は設計 83 §10 の増分で入れる。`USERID` は認可の設計に合わせる。 |
