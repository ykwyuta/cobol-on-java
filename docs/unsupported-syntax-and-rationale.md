...
| **CICS START の一部 (`TERMID` / `USERID` / `CHANNEL`)** | `EXEC CICS START TRANSID('TX02') TERMID(EIBTRMID) END-EXEC` | 端末を持たない task を起こす `START` と、`RETRIEVE`、`CANCEL REQID` は実装済み（設計 82 §6、P-138）。`PROTECT` も実装済み（P-141）。端末へ出す task、別の利用者で動かす `USERID`、`CHANNEL` / `ATTACH`、`RETRIEVE WAIT`、POST の `CANCEL` は、端末の adapter・task の境界・利用者の認可と合わせた設計を持たないので断る。 | 端末の adapter と task の境界の commit に合わせて設計する。 |
