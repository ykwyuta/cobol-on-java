...
| **CICS file control の `SET`** | `EXEC CICS READ FILE('F') SET(PTR) RIDFLD(K) END-EXEC` | KSDS / RRDS / ESDS / BDAM の READ / WRITE / REWRITE / DELETE / UNLOCK と browse、`RBA` / `XRBA`、`TOKEN`、`NOSUSPEND`、`CONSISTENT` / `REPEATABLE`、`MASSINSERT`、`DEBKEY` / `DEBREC`、構成した自 region の `SYSID` は暫定の仕様で実装済み（設計 82 §3、設計 85 §4・§5、P-136、P-147）。`SET` は CICS が持つ域への pointer で、置き場の有効な期間 (READ の頁) の設計を入れるまで断る。文書が条件を示さない形は失敗させる。 | `ADDRESS OF` (P-150) の番号の上に、設計 85 §5.5 の置き場を足す。 |
| **CICS キューの一部 (`SET`、`ITEM` も `NEXT` も無い `READQ TS`)** | `EXEC CICS READQ TS QUEUE('Q') SET(PTR) NEXT END-EXEC` | `WRITEQ` / `READQ` / `DELETEQ` の `TS` / `TD` は 1 つの JVM の中のキューとして実装済み（設計 82 §4・§5、P-137）。trigger level による task の開始は設計 83 (P-144)、区画外の TD (順編成のデータセット) と回復可能なキュー (STRICT の UOW に入る) は設計 85 §7 (P-148)、構成した自 region の `SYSID` と `NOSUSPEND` は設計 85 §4 (P-149) で実装済み。`SET` と、`ITEM` も `NEXT` も無い `READQ TS` は断る。 | `ADDRESS OF` (P-150) の番号の上に、設計 85 §5.5 の置き場を足して `SET` を入れる。 |
| **CICS START の一部 (`USERID` と `TERMID` の併記、`RETRIEVE SET`)** | `EXEC CICS START TRANSID('TX02') USERID('U1') TERMID('T001') END-EXEC` | 端末を持たない task を起こす `START` と、`RETRIEVE`、`CANCEL REQID` は実装済み（設計 82 §6、P-138）。`PROTECT` も実装済み（P-141）。端末へ出す `TERMID` は `JdbcCicsStarts` の構成で実装済み（設計 83 §5、P-144）。`RETRIEVE WAIT` も端末へ出す task で実装済み。別の利用者で動かす `USERID` は代理の権限を確かめて実装済み（設計 84、P-145）。`CHANNEL` / `ATTACH` / `NOCHECK` / `SYSID`、REQID の無い `CANCEL` (POST を持たないので NOTFND) は暫定の仕様で実装済み（設計 85 §8、P-149）。`USERID` と `TERMID` の併記と `RETRIEVE SET` は断る。 | 併記は実機の振る舞いを確かめて設計する。`SET` は `ADDRESS OF` のあとに足す。 |
