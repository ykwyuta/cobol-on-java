...
| **CICS START の一部 (`TERMID` / `PROTECT` / `USERID` / `CHANNEL`)** | `EXEC CICS START TRANSID('TX02') TERMID(EIBTRMID) END-EXEC` | 端末を持たない task を起こす `START` と、`RETRIEVE`、`CANCEL REQID` は実装済み（設計 82 §6、P-138）。端末へ出す task、同期点まで遅らせる `PROTECT`、別の利用者で動かす `USERID`、`CHANNEL` / `ATTACH`、`RETRIEVE WAIT`、POST の `CANCEL` は、端末の adapter・task の境界・利用者の認可と合わせた設計を持たないので断る。 | 端末の adapter と task の境界の commit に合わせて設計する。 |
