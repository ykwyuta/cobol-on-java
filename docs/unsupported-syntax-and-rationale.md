...
| **CICS file control の `SET`** | `EXEC CICS READ FILE('F') SET(PTR) RIDFLD(K) END-EXEC` | KSDS / RRDS / ESDS / BDAM の READ / WRITE / REWRITE / DELETE / UNLOCK と browse、`RBA` / `XRBA`、`TOKEN`、`NOSUSPEND`、`CONSISTENT` / `REPEATABLE`、`MASSINSERT`、`DEBKEY` / `DEBREC`、構成した自 region の `SYSID` は暫定の仕様で実装済み（設計 82 §3、設計 85 §4・§5、P-136、P-147）。`SET` は CICS が持つ域への pointer で、`ADDRESS OF` が無いので断る。文書が条件を示さない形は失敗させる。 | 設計 85 §9 の順に `ADDRESS OF` を入れてから足す。 |
| **CICS キューの一部 (`SET`、`ITEM` も `NEXT` も無い `READQ TS`)** | `EXEC CICS READQ TS QUEUE('Q') SET(PTR) NEXT END-EXEC` | `WRITEQ` / `READQ` / `DELETEQ` の `TS` / `TD` は 1 つの JVM の中のキューとして実装済み（設計 82 §4・§5、P-137）。trigger level による task の開始は設計 83 (P-144)、区画外の TD (順編成のデータセット) と回復可能なキュー (STRICT の UOW に入る) は設計 85 §7 (P-148)、構成した自 region の `SYSID` と `NOSUSPEND` は設計 85 §4 (P-149) で実装済み。`SET` と、`ITEM` も `NEXT` も無い `READQ TS` は断る。 | 設計 85 §9 の順に `ADDRESS OF` を入れてから `SET` を足す。 |
