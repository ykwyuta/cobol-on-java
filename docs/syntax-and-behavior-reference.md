...
- **振る舞い**:
  ...
  - file control の暫定の仕様 (設計 85 §4・§5、暫定判断 P-147): ESDS は到着順に置き、`WRITE` は終わりに足して RIDFLD に RBA (`RBA` は 4 byte、`XRBA` は 8 byte) を返します。RBA は前にある record の長さの和で、実機の値とは一致しません。`TOKEN` は同じ file の複数の record を持ち、合わない token は `INVREQ` 47 です。`NOSUSPEND` は持たれた record を待たず `RECORDBUSY` 107、`CONSISTENT` / `REPEATABLE` は持たれた record を待ちます (RLS の区別を持たず、どの file でも受けます)。`MASSINSERT` は普通の `WRITE` です。BDAM は RIDFLD の先頭 4 byte の相対 block 番号を RRDS の番号に写し、`DEBREC` は相対 record 番号 0 だけ、`DEBKEY` は鍵が一致する record だけを読みます。`SYSID` は region に構成した名前だけを自 region として処理し、ほかは `SYSIDERR` 130 です。`SET` は断ります。
  - 一時記憶のキュー (`WRITEQ` / `READQ` / `DELETEQ TS`) は定義を要らず、region の中で task どうしが分け合います。`READQ TS NEXT` は直前に読まれた item の次を読み、終わりは `ITEMERR` です。一時データのキュー (`TD`) は region で定義した区画内のキューで、先に書いた record から取り出し、空なら `QZERO` です。遠隔のキューは持ちません (設計 82 §4・§5、暫定判断 P-137)。区画外のキューはバッチの順編成のデータセットに置き、`OUTPUT` のキューへ書いて `INPUT` のキューを順に読みます。向きの違う命令と `DELETEQ TD` は `INVREQ`、開けないデータセットは `NOTOPEN` です。回復可能なキュー (`RECOVSTATUS=LOGICAL`) は STRICT の task の業務の UOW に入り、同期点の commit で確定し、ROLLBACK と ABEND で取り消します。他の task は `QBUSY` を受けずに commit まで待ちます (設計 85 §7、暫定判断 P-148)。
...
- `EXEC SQL INCLUDE 名前 END-EXEC` は前処理で `COPY 名前` と同じに取り込みます。`SQLCA` は置き場に無ければ公開の宣言 (136 byte) から作ります。`SQLDA` は公開文書の欄の説明から作った暫定の形 (SQLVAR 750 個、番地は 4 byte の POINTER) で置きます。動的 SQL が無いので宣言として置くだけです (暫定判断 P-146)。
