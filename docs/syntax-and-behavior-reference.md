...
EXEC CICS START TRANSID(名前) [INTERVAL(hhmmss) | TIME(hhmmss) | (AFTER | AT) [HOURS(n)] [MINUTES(n)] [SECONDS(n)]] [FROM(域) [LENGTH(n | 項目)]] [REQID(名前)] [RTRANSID(名前)] [RTERMID(名前)] [QUEUE(名前)] [PROTECT] END-EXEC
