...
| :--- | :--- |
| **作成日** | 2026-09-22 |
| **更新日** | 2026-09-22 (§5 の未確認事項を調査し、結果と訂正を反映。コーパスの方針を §5.6 に決めた。出典は §12) |
...
>
> **コーパスの方針を決めた (§5.6、暫定判断 P-176)。**一貫した業務資産を待たず、
> **小さな実例を積み重ねて通すべき資産とする。**CCVS85 が 458 本の小さな検査で
> COBOL の順番を決めてきたのと同じ方法であり、妥協ではない。
> あわせて、前回「公開資産は小さな実例だけ」と書いたのは**誤りだった**。
> Apache-2.0 の ILE RPG で書かれた会計系 ERP (`intERPrise`) が存在する (§5.6.1)。
...
**そのあと、コーパスを決める作業 (§5.6) でもう 1 つ訂正が出た。**
「公開されている RPG の資産は小さな実例だけ」も誤りであり、Apache-2.0 の ILE RPG で
書かれた会計系 ERP が存在した (§5.6.1)。**この節の調査は 2 度とも自分の記述を訂正している。**
...
### 5.6 通すべき資産 — 小さな実例を積んでコーパスにする

**利用者の決定 (2026-09-22): 一貫した業務資産を待たず、小さな実例を積み重ねて
通すべき資産とする。**暫定判断 P-176 に記録した。

この決定は「妥協だから仕方なく」ではなく、**この処理系が既に採っている方法そのもの**である。
CCVS85 は 458 本の小さな検査プログラムの集まりであって、1 本の業務資産ではない。
NIST CCVS85 の数 (受理率 96.9%) を出しているのは、まさに「小さな実例の積み重ね」である。

ただし**小さな実例で決められることと決められないことがある**ので、先に分けておく。

#### 決められること — 広さ

**何を実装するかの順番は、小さな実例で決まる。**CLAUDE.md の増分の型の第 2 段
「いちばん大きい塊を選ぶ」は、資産が 1 本でも 300 本でも同じように働く。むしろ
**数が多いほうが塊は正確に出る**。1 本の業務資産は、その 1 本が使う機能に偏るからである。

やり方は `cobol-verify` が COBOL でやっていることと同じである。

1. コーパスを翻訳し、**止まった理由ではなく止まった原文の行**を集める (CLAUDE.md §4)
2. 演算子・組込み関数・キーワード・命令の**出現数**を数える
3. 多い順に実装する

RPG は COBOL より機能の数が多いので、この数え方が**なおさら効く**。
`%SCAN` と `%LOOKUP` のどちらを先に書くかは、意見ではなく数で決まる。

#### 決められないこと — 深さと相互作用

**小さな実例は、層をまたぐ振る舞いを測れない。**これは正直に書いておく必要がある。

| 測れないもの | なぜ |
| :--- | :--- |
| RLA とコミットメント制御の相互作用 | 実例は 1 つのファイルを読んで終わる。同期点をまたがない |
| レコードロックの競合 | 実例は 1 つのジョブしか動かさない |
| 表示ファイルと DB の往復 | サブファイルを埋めて更新して再表示する流れが要る |
| RPG サイクルの全体 | 実例はほぼ `*NOMAIN` か線形の主処理である |
| 量による差 (開き直し、位置づけの保持) | 実例のデータは数十件である |
| ILE の活性化グループの有効範囲 | 実例はプログラム 1 本で閉じる |

§4.1 で「並行更新下で `READE` の位置がどうなるかは実機で測る以外に決めようがない」と
書いたが、**小さな実例を何本集めてもこれは出てこない**。出てこないものを
「出なかったから問題ない」と読むのが、いちばん危ない。

したがって**この層は自分で書く**。実機 (§5.2) に対して、こちらで組んだ小さな試験を
ぶつけて期待値を採る。`cobol-oracle` が Hercules に対して合成ジェネレータ (NFR-041) で
命令列を作って流しているのと**同じ形**である。コーパスを流すのとは別の道具になる。

> **数の意味を混ぜない。**コーパスの通過率は「広さ」の数であり、
> 合成試験の一致率は「深さ」の数である。1 つの率にまとめない。

#### ライセンス — 調べた結果、ほとんど宣言が無い

積むと決めた以上、**出所を確かめた**。結果は良くない。

| 資産 | ライセンス |
| :--- | :--- |
| `SJLennon/IBM-i-RPG-Free-CLP-Code` | **宣言なし** |
| `patriciocostilla/rpg-examples` | **宣言なし** |
| `richardschoen/rpgclcodingexamples` | **宣言なし** |
| `IBM/rpg-genai-data` | **Apache-2.0** |
| `lppedd/RPG` | **MIT** |
| `i-ntERPrise/intERPrise` | **Apache-2.0** (§5.6.1) |

**ライセンスの宣言が無いものは、既定で全権利が留保されている。**再配布はできない。

ただし**このプロジェクトは元からコーパスを同梱しない** (要件 NFR-042)。取得元・版・
SHA-256 を manifest に固定して取ってくる。CCVS85 と OSS コーパスで既にそうしている。
つまり**必要な仕組みは既にある**。

したがって規則はこうなる。

1. **同梱しない。**manifest に URL と SHA-256 を書き、取得スクリプトで持ってくる
2. **写さない。**コーパスの原文をテストの中へ貼らない。診断の引用は行番号と短い抜粋に留める
3. **宣言の無いものは、測るためだけに使う。**通過率の材料にはするが、再配布物には入れない
4. **宣言のあるものを優先して積む。**Apache-2.0 と MIT のものから始める

#### 5.6.1 見落とし — 一貫した業務資産は存在した

**初版と前回の更新で「公開資産は小さな実例だけ」と書いたのは、調べ方が足りなかった。**

`i-ntERPrise/intERPrise` は **Apache-2.0 の、自由形式 ILE RPG で書かれた ERP** である。

| 項目 | 内容 |
| :--- | :--- |
| ライセンス | **Apache-2.0** |
| 言語 | **自由形式 ILE RPG** (ILE C / COBOL の寄与も受け付ける) |
| データベース | **DB2 for i**。トリガ、制約、コピーブックを持つ。応用のモデルは約 898 表 |
| 業務範囲 | **総勘定元帳、買掛、売掛、現金出納** |
| 構造 | 多層 (MVC)。`db.services` / `transport.services` / `ui.services` に分かれる |
| 状態 | ALPHA 0.2。約 106 コミット |

**業務の範囲が Bank-of-Z と近い。**勘定と取引を持つ会計系であり、
「一貫した業務資産が無い」という前回の判断は**誤りだった**。

ただし過大評価もしない。

- **ALPHA 0.2 である。**完成した製品資産ではない
- **多層である。**UI が PHP なので、RPG 側だけを切り出す必要がある
- **表が約 898 ある。**全部を通す話ではなく、どこから切るかを決める話になる
- **中身を読んでいない。**行数、使っている機能、DDS か SQL DDL か、サブファイルを使うかは
  **まだ確かめていない**。増分 0c で確かめる

したがって結論は「小さな実例を積む」を**置き換える**のではなく、**層を足す**ことになる。

#### 5.6.2 コーパスを 3 層に分ける

| 層 | 中身 | 何を測るか | 状態 |
| :--- | :--- | :--- | :--- |
| **広さ** | 小さな実例の積み重ね (§5.6) | どの機能がどれだけ使われるか。実装の順番 | **これから積む** |
| **一貫性** | `intERPrise` (Apache-2.0) | 業務が最後まで通るか。層をまたぐ流れ | **Bank-of-Z の位置。要調査** |
| **深さ** | 自分で組む合成試験 + 実機 | RLA の位置づけ、ロック、同期点、量 | **自分で書く。コーパスではない** |

3 つは**別々に数える**。まとめた 1 つの率にしない (CLAUDE.md §7)。
...
| **0c** | **コーパスを積む** (§5.6)。小さな実例を manifest (URL + SHA-256) で固定し、`rpg-verify` の実行器で状態別に数える。**同梱しない** | 数が出る。実装の順番が数で決まる |
| **0d** | **`intERPrise` を読む** (§5.6.1)。行数、使っている機能、DDS か SQL DDL か、サブファイルの有無を数える | 「一貫性」の層に使えるかが判明する |
...
**増分 0b を 0c より先にするのは、実機に触れないと採取の形が決まらないからである。**

**そして 0c は「資産が決まるまで待つ」ではない。**小さな実例を積んだ時点で、
実装の順番を決める数は出る (§5.6)。CCVS85 が 458 本の小さな検査で COBOL の順番を
決めたのと同じである。**待つ理由が無い。**

0d を別に置いたのは、`intERPrise` が使えるかどうかがまだ分からないからである。
使えるなら「一貫性」の層に入り、使えないなら広さと深さの 2 層で進める。
**どちらでも 0c は止まらない。**
...
### 10.2 「通すべき資産」は解いた — ただし前回の判断は誤りだった

**前回の更新で「公開資産は小さな実例だけなので、この問いは調査では解けない」と書いたのは
誤りだった。**調べ方が足りなかった。§5.6.1 のとおり、`i-ntERPrise/intERPrise` という
**Apache-2.0 の、自由形式 ILE RPG で書かれた会計系 ERP** が存在する。
総勘定元帳・買掛・売掛・現金出納を持ち、**業務の範囲は Bank-of-Z と近い**。

そのうえで、利用者の決定 (§5.6、暫定判断 P-176) により方針はこうなった。

**一貫した業務資産を待たず、小さな実例を積み重ねて通すべき資産とする。**

これは妥協ではない。CCVS85 が 458 本の小さな検査プログラムの積み重ねであり、
この処理系の COBOL の順番をそれが決めてきたのと同じ方法である。
**実装の順番を決めるのに、1 本の大きな資産は要らない。**むしろ 1 本に偏る。

したがってコーパスは 3 層になる (§5.6.2)。

| 層 | 役割 | 由来 |
| :--- | :--- | :--- |
| 広さ | 実装の順番を数で決める | **小さな実例の積み重ね (利用者の決定)** |
| 一貫性 | 業務が最後まで通るか | `intERPrise` (要調査、増分 0d) |
| 深さ | 層をまたぐ振る舞い | **自分で組む合成試験 + 実機** |

**残っているのは「深さ」である。**小さな実例も `intERPrise` も、RLA とコミットメント制御の
相互作用、ロック競合、量による差は測れない (§5.6 の表)。ここは**コーパスでは解けない**ので、
自分で試験を組んで実機にぶつける。これは未決事項ではなく、**やることが決まっている仕事**である。
...
**通すべき資産については、方針が決まり、前回の判断が 1 つ覆った。**利用者の決定により、
一貫した業務資産を待たず**小さな実例を積み重ねてコーパスとする** (§5.6、暫定判断 P-176)。
これは妥協ではなく、CCVS85 が 458 本の小さな検査で COBOL の順番を決めてきたのと同じ方法である。
実装の順番を決めるのに大きな資産は要らない。

同時に、前回「公開資産は小さな実例だけ」と書いたのは**誤りだった**。
`i-ntERPrise/intERPrise` は Apache-2.0 の自由形式 ILE RPG で書かれた会計系 ERP
(総勘定元帳・買掛・売掛・現金出納、約 898 表) であり、**業務の範囲は Bank-of-Z と近い**。
中身は未調査なので、増分 0d で確かめる。

そこでコーパスを 3 層に分けた (§5.6.2)。**広さ**は小さな実例、**一貫性**は `intERPrise`、
**深さ**は自分で組む合成試験と実機である。**3 つは別々に数える。**
小さな実例をいくら積んでも、RLA とコミットメント制御の相互作用やロック競合は出てこない。
**出てこないものを「出なかったから問題ない」と読まない。**

**次にやること**は、増分 0b (実機への口を作る)、0c (コーパスを積んで数を出す)、
0d (`intERPrise` を読む) である。**0c は資産が決まるのを待たない。**
...
**RPG の公開資産とそのライセンス (2026-09-22 に各リポジトリで確認)**

一貫した業務資産 (§5.6.1):

- [i-ntERPrise/intERPrise](https://github.com/i-ntERPrise/intERPrise) — **Apache-2.0**。
  自由形式 ILE RPG の ERP。総勘定元帳・買掛・売掛・現金出納。DB2 for i、約 898 表、
  多層 (`db.services` / `transport.services` / `ui.services`)、ALPHA 0.2、約 106 コミット
- [New Open Source ERP Suite Written In ILE RPG (IT Jungle, 2018)](https://www.itjungle.com/2018/10/15/new-open-source-erp-suite-written-in-ile-rpg/) — 経緯

ライセンスが宣言されているもの (優先して積む):

- [IBM/rpg-genai-data](https://github.com/IBM/rpg-genai-data) — **Apache-2.0**
- [lppedd/RPG](https://github.com/lppedd/RPG) — **MIT**

**ライセンスの宣言が無いもの** (測るためだけに使う。再配布物に入れない。§5.6):

- [SJLennon/IBM-i-RPG-Free-CLP-Code](https://github.com/SJLennon/IBM-i-RPG-Free-CLP-Code) — 宣言なし。
  自由形式 RPGLE / SQLRPGLE / CLLE / DDS。5250 の顧客保守、サブファイル、レコードロック、
  出力仕様書によらない印刷などの実例
- [patriciocostilla/rpg-examples](https://github.com/patriciocostilla/rpg-examples) — 宣言なし。
  `cl ile` / `database-files-examples` / `rpg ile` / `rpg3` / `sql-db2-examples`。
  順次アクセスまで実装され、索引アクセスと帳票は未着手
- [richardschoen/rpgclcodingexamples](https://github.com/richardschoen/rpgclcodingexamples) — 宣言なし。
  7〜8 本。レコードの存在検査、JSON、表示属性、SQL の CLOB など
